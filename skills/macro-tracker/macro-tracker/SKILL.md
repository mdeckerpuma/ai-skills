...
- **Stay inside** `C:\Users\mdecker\ai-skills\macro-tracker\` for all data files — this ensures everything is backed up to GitHub.
...
- `C:\Users\mdecker\ai-skills\macro-tracker\logs\YYYY-MM-DD.json` — one file per day, append-only meal log
- `C:\Users\mdecker\ai-skills\macro-tracker\profile.json` — user's fitness goal, stats, and calculated daily calorie target
- `C:\Users\mdecker\ai-skills\macro-tracker\progress-reports\YYYY-MM-DD-report.md` — weekly progress reports
...
1. **Load profile**
   Read `C:\Users\mdecker\ai-skills\macro-tracker\profile.json`.
...
4. **Append to today's log**
   Open `C:\Users\mdecker\ai-skills\macro-tracker\logs\YYYY-MM-DD.json` (create if it doesn't exist).
   Append a new entry:
   ```json
   {
     "time": "HH:MM",
...
When the user says "progress report", "weekly summary", or "how am I doing", generate a report and save it to `C:\Users\mdecker\ai-skills\macro-tracker\progress-reports\YYYY-MM-DD-report.md`.

Report format:
```
# Macro Tracker Progress Report
Generated: [date]
Period: [date range covered]

## Averages (last 7 days)
- Calories: X / Y kcal avg (Z% of goal)
- Protein: Xg avg
- Carbs: Xg avg
- Fat: Xg avg

## Highlights
- Best day: [date] — hit X% of goals
- Most consistent macro: [macro]
- Biggest gap: [macro] — averaged Xg below target

## Micronutrient Trends
- Sodium, Potassium, Calcium, Iron, Vitamin C, Vitamin D averages

## Recommendations
- [1-3 specific, actionable notes based on the data]
```
...
```
C:\Users\mdecker\ai-skills\macro-tracker\
├── SKILL.md
├── profile.json                        ← created on first run
├── logs\
│   └── YYYY-MM-DD.json                 ← one per day, append-only
└── progress-reports\
    └── YYYY-MM-DD-report.md            ← generated on demand
```
